<script>
  ...
  <div class="carousel-container">
    ...
    <div>
        <a href={`/lecciones/fibonacci2/${(data.body.id == 1) ? 1 : data.body.id-1}`}>Anterior</a>
        <a href={`/lecciones/fibonacci2/${ (data.body.id ==data.body.maxImage)? data.body.maxImage : (parseInt(String(data.body.id)) +1) }`}>Siguiente</a>
      
      {#if data.body.id ==data.body.maxImage}
      <a href={`/quizf/quiz/`}>Ir al Quiz</a>
      {/if}
    </div>
  </div>